...
api.mapkey('gh', '#8Open URL from history', function() {
    api.Front.openOmnibar({type: "History"});
});
api.mapkey('M', '#8Open URL from vim-like marks', function() {
...
api.map(']', 'R');
api.map('[', 'E');
// mapkey('<F1>', '#3Go one tab left', 'RUNTIME("previousTab")');
// mapkey('<F2>', '#3Go one tab right', 'RUNTIME("nextTab")');
// mapkey('a', '#3Go one tab left', 'RUNTIME("previousTab")');
api.map('F', 'af');
api.map('u', 'X');
api.map('H', 'S');
api.map('w', 'T');
...
api.addSearchAlias('cd', 'cambrige dictionary', 'http://dictionary.cambridge.org/search/american-english/direct/?source=gadgets&q=');
api.addSearchAlias('kino', 'kinopoisk', 'http://www.kinopoisk.ru/index.php?first=no&what=&kp_query=');
api.addSearchAlias('gt', 'Translate', 'http://translate.google.com/?source=osdd#auto|auto|');
